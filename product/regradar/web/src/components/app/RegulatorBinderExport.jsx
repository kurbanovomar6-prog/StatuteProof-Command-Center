...
/**
 * Regulator-ready evidence binder export.
 *
 * @param {{ sources?: { id: string, name: string }[] }} props
 *   `sources` is derived by the parent from loaded evidence records, so the
 *   picker only offers sources the caller actually has evidence for.
 */
export default function RegulatorBinderExport({ sources = [] }) {
  ...
  // Derive the effective source so a source list that arrives after mount still
  // has a sensible default, without a syncing effect.
  ...
  async function handleExport() {
    ...
    setMessage('')
    try {
      const { blob, filename } = await reports.regulatorBinder({
      ...
      setMessage(`Binder downloaded — ${filename} (${kb} KB)`)
      setStatus('success')
    } catch (err) {
      if (err.empty) {
        setStatus('empty')
        setMessage('No captured changes for this source in the selected period.')
      } else {
        setStatus('error')
        setMessage(err.message || 'Could not export the binder.')
      }
    }
  }
  ...
  return (
    <div className="rounded-xl border border-cyan-400/20 bg-[#0D1B2E] p-5">
      <div className="mb-4 flex items-start gap-3">
        ...
        <div>
          <h2 className="text-sm font-semibold text-white">Evidence binder for regulatory review</h2>
          <p className="mt-0.5 max-w-2xl text-xs leading-relaxed text-slate-400">
            A timestamped, hash-verified pack of every change StatuteProof captured on this source
            for the period — with an offline verifier the examiner can run without trusting us.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {isWorking ? 'Building binder…' : 'Export binder'}
        </button>
      ...
      <div className="mt-4 flex items-start gap-2 rounded-lg border border-slate-800 bg-slate-950/35 px-3 py-3">
        ...
        <p className="text-[11px] leading-relaxed text-slate-500">
          The binder contains the sealed evidence records, their raw and normalized snapshots, the
          diffs, and a standalone verify.py. For monitoring information only. Not legal advice and
