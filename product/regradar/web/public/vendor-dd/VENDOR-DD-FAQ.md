...
**Last reviewed:** 2026-07-25. Where an answer cites a module or config file,
...
**no RFC 3161 timestamping authority** — that code exists but is dormant and
**not currently enabled in production**, so no TSA is contacted and none is a
sub-processor today (earlier revisions of this pack listed one in error;
corrected 2026-07-25); off-box backup storage — **required in code** since 2026-07-20, so
...
restarts (`deploy/systemd/`). Both of those detectors run on the single
production host, so they catch a wedged or diverging *service* but cannot page
anyone if the droplet itself dies; that gap is covered only by an optional
external probe URL the operator supplies (`STATUTEPROOF_HEARTBEAT_PING_URL` —
see `SECURITY-OVERVIEW.md` §9), which the deploy gate warns about rather than
requires. Ask us whether it is set. There is no 24×7 SOC or on-call rotation, and no
