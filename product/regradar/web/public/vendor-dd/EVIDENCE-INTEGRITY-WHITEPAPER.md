...
**Last reviewed:** 2026-07-25. This is a buyer-facing summary of the full open
...
why the separately persisted head anchor (§4) exists, and why the external
RFC 3161 anchor (§7) was built, though it is not currently enabled.
...
- **Daily integrity self-check (implemented; required by the deploy gate):** a
  ...
  Unlike the self-seal and the head anchor, which are properties of the code
  and hold wherever it runs, this one is a property of the *deployment*: it
  runs because a systemd timer is enabled on the host. `deploy/deploy-check.sh`
  fails the deploy when that unit is not enabled and active, which is how the
...
- **The limit:** an actor with full write access to the store who re-seals and
  ...
  anchored head), and is the backstop that is actually in force today. The
  external RFC 3161 anchor (§7) would raise it further with a signature
  StatuteProof cannot forge — but it is built and not enabled, so **do not
  count it among the protections currently in force**. We do not claim
  "immutable" or "tamper-proof", and any StatuteProof material that did would
  be wrong.

## 7. External RFC 3161 timestamp anchor — built and tested, NOT currently enabled
...
- **Status: implemented and covered by tests, but not currently enabled in
  production.** The anchor is dormant by default and activates only when the
  operator sets `RFC3161_TSA_URL` (`app/config.py` defaults it to empty). That
  variable is **not set on the production host**, so today the anchor is a
  complete no-op: no network calls, no threads, no files, no `.tsr` tokens, and
  nothing TSA-related in any evidence pack.
  **Correction (2026-07-25):** revisions of this document dated 2026-07-18
  stated the anchor was enabled in production against a named public TSA. That
  was wrong — no TSA was ever configured on the live host. The statement is
  withdrawn here, and the same correction is applied to `SECURITY-OVERVIEW.md`,
  `DATA-FLOW-AND-RESIDENCY.md` and `VENDOR-DD-FAQ.md`.
  **What this means for you:** the guarantee to rely on is the one in §6 —
  tamper-evident, backed by the self-seal, the hash chain and the separately
  persisted head anchor. Do not credit this system with third-party timestamp
  attestation in your assessment. If an independent "when" attestation is a
  requirement for you, say so: turning it on is operator configuration against
  code that already exists and is tested, not a development project.
- **What it would add once enabled:** the TSA signs a token binding the head
...
- **What would be sent:** a single SHA-256 digest. No content, no personal data.
- **Independent verification (once enabled):** the token is persisted both as
  JSON metadata and as a raw DER `.tsr` file readable by standard `openssl ts`
  tooling; the spec (§6.3) gives the exact `openssl ts -verify` commands. The
  bundled offline verifier checks the imprint and the TSA signature against the
  ...
  store) — never a false pass. This verification path is exercised by the test
  suite against generated tokens; it has not yet run against a live TSA.
