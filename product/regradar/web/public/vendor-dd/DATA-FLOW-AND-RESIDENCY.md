...
**Last reviewed:** 2026-07-25. Claims are grounded in the referenced modules
...
| Flow | Data | Module |
|---|---|---|
| Scheduled monitoring fetches | Public regulator web pages and documents (140 enabled sources at time of writing; "enabled" means fetched, not necessarily alert-eligible — see `SECURITY-OVERVIEW.md` §1) | `app/scraper.py`, `app/adapters/*`, `sources.json` |
...
| RFC 3161 Time-Stamping Authority | Only when `RFC3161_TSA_URL` is set (dormant by default in code). **Not currently enabled in production** — the variable is unset on the live host, so this flow does not occur at all today | A single SHA-256 hash (the evidence chain head) — no content, no personal data — *if it were enabled* | `app/rfc3161_anchor.py` |
...
| RFC 3161 TSA (operator-selected) | External timestamp anchoring of the evidence chain head | One SHA-256 hash; no content, no personal data | Only if `RFC3161_TSA_URL` is configured — dormant by default in code and **not currently enabled in production**, so there is **no TSA sub-processor today**. Earlier revisions of this pack (2026-07-18) listed one in error; corrected 2026-07-25 |
