...
| `current_hash` (normalized) | The hash of the content-stable form, where `normalized.txt = normalize_for_change_hash(raw)` (strips volatile markup/whitespace so a cosmetic republish is not a false change). Stored as `content.current_hash`. **Two flavors exist and a verifier accepts either:** intake/canonical records use `SHA-256( bytes of normalized.txt )` (newline-preserving); the live monitoring pipeline uses `stable_content_hash(normalized.txt)` (a further whitespace-collapse). Because `normalized.txt` *is* `normalize_for_change_hash(raw)`, `stable_content_hash(normalized.txt)` reproduces the pipeline flavor exactly — so a genuine record of either flavor verifies against its own `normalized.txt`, while a tampered `normalized.txt` matches neither. |
...
| `diff_hash` | `SHA-256( bytes of diff.txt )` — the sealed redline (the line-level added/removed text) for a `CHANGED` record. Stored as `content.diff_hash`, so it is covered by `record_hash`. **Additive:** records captured before this field existed, and non-`CHANGED` records with no diff, simply omit it; a verifier skips the diff check when it is absent. |
| `captured_at` / `source_url` | The capture timestamp and official source URL, stored INSIDE `content` (as `content.captured_at` / `content.source_url`) so they are covered by `record_hash` — the **when** and **where** of the capture cannot be altered without breaking the seal. They duplicate the display copies `run.timestamp` / `source.official_url`; a verifier cross-checks the two and fails on a mismatch. **Additive:** legacy records omit them and skip the cross-check. |
...
2. **Recompute the normalized hash:** `sha256sum normalized.txt` → must equal
   `record.current_hash`.
3. **Recompute the diff hash (CHANGED records):** if the record carries
...
5. **Recompute the record hash:** for a canonical `evidence-record.json`
   ...
   `sha256:` prefix). Because `content` includes `current_hash`, `raw_hash`,
   `diff_hash` (when present), and the previous-side fields, this one fingerprint
   covers every hash the record asserts: altering any field inside `content` makes
   `record_hash` diverge. (For a source-run trail record, the record hash is instead
   computed over its identifying fields; see §4.)
