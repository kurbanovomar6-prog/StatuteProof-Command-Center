...
| Evidence-library only | 4 |
| Candidate | 9 |
| Remediation | 45 |
| Source-level MONITOR_OK | 75 |
...
| VARA | 7 | 2 | 1 | 0 | 1 | Partial | 23 |
| DFSA | 16 | 1 | 1 | 0 | 10 | Partial Production Access | 24 |
| DIFC | 12 | 8 | 0 | 0 | 1 | Good | 17 |
| ADGM/FSRA | 14 | 9 | 0 | 3 | 0 | Good | 16 |
| Ministry of Economy / DNFBP AML | 9 | 0 | 1 | 0 | 7 | Remediation | 25 |
| FTA | 6 | 0 | 0 | 5 | 0 | Partial | 25 |
...
| Ministry of Justice / UAE Legislation / Gazette | 3 | 0 | 0 | 0 | 0 | Partial | 25 |

Family fresh-alert counts above apply the FULL readiness gate (mode + alert-eligible + MONITOR_OK +
proof + baseline + daily cadence), so they can sit below the headline mode-eligible count while the
2026-07-18 promotions (DFSA AML rulebook module, VARA Compliance & Risk Management Rulebook) await
their registry readiness-field sync from the next production intake.
...
- VARA: 2 VARA sources pass the full readiness gate (news/circulars, enforcement). The Compliance and Risk Management Rulebook (incl. AML/CFT Part III) was promoted to fresh_alert on 2026-07-18 after two stable production runs and counts in the register headline; its registry proof fields sync on the next intake. The 30-day revision-updates view is in remediation pending a production rebaseline. This is selected-source monitoring, not complete VARA coverage.
- DFSA: The 10 www.dfsa.ae sources are held in remediation as a production-egress disclosure, not a content problem (HTTP 403 to our production egress on all methods since 2026-07-11). DFSA depth continues via the dfsaen.thomsonreuters.com rulebook platform, which does work from production — the rulebook source passes the full readiness gate and the AML rulebook module was promoted to fresh_alert on 2026-07-18 after rebaseline plus stable-run gates (its registry readiness fields sync on the next intake).
- DIFC: DIFC has 8 fresh-alert sources including the official DIFC Legal Database listing; the laws-and-regulations root listing moved to remediation on 2026-07-18 after three consecutive QUALITY_DROP production runs (pending audit-record review and a production rebaseline). This is selected DIFC monitoring, not complete DIFC legal database coverage.
- ADGM/FSRA: ADGM/FSRA has 9 fresh-alert sources including guidance/policy and RA/circular/rulebook style sources. Three candidate rows remain held until selectors/extraction pass the same gates.
- Ministry of Economy / DNFBP AML: All 7 MoE DNFBP sources remain under source remediation — earlier captures were a site-maintenance page, which the quality gate rejects; none is counted until re-baselined on real content. One evidence-library homepage is kept as a snapshot only.
- FTA: No FTA sources are currently fresh-alert eligible. FTA portal/listing extraction remains candidate/adapter work and is not counted as fresh-alert.
...
- Ministry of Justice / UAE Legislation / Gazette: MoJ/Gazette has no fresh-alert eligible sources currently. Root portal, e-Laws, and complete Official Gazette coverage remain unclaimed.
