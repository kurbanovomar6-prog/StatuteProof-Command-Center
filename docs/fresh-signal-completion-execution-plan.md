...
At the start of that pass, `sources.json` had no `monitoring_mode` or `alert_eligible` fields on the then-226 enabled UAE rows. The later fresh-source expansion and VARA enforcement activation superseded this with the current 238 enabled / 169 fresh-alert source model.
